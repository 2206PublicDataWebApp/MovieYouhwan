    ...
    <ul class="pagination">
      ...
      <c:forEach var="page" begin="${nPageInfo.startPage }" end="${nPageInfo.endPage }">
        ...
        <c:if test="${nPageInfo.currentPage ne page}">
          <li><a href="/admin/site/manage.yh?nCurrentPage=${page}&tabIndex=2">${page}</a></li>
        </c:if>
      </c:forEach>
      <c:if test="${nPageInfo.endPage ne nPageInfo.pageCount}">
        <li><a href="/admin/site/manage.yh?nCurrentPage=${nPageInfo.endPage+1}&tabIndex=2">&raquo;</a></li>
      </c:if>
    </ul>
  </div>
</div>
